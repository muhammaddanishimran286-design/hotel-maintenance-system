...
p('Testing was completed for the Hotel Maintenance Management System, a Laravel-based web application that supports authentication, dashboard monitoring, maintenance request management, task assignment, task status progression, notifications and profile management. The assessment combined manual functional validation, visual verification of the running application, source-level traceability and the existing automated regression suite.')
table(['Measure','Result'],[['Documented functional test cases','18'],['Functional test cases passed','18'],['Functional test cases failed','0'],['Functional test cases blocked/not executed','0'],['Functional coverage','100%'],['Automated regression tests','25 passed (62 assertions)'],['Open critical or major defects','0'],['Overall readiness','READY FOR DEMONSTRATION']],[3.2,3.0])
...
table(['Technique','Purpose','Evidence'],[['Positive functional testing','Confirm intended workflows succeed','TC-001, TC-003, TC-004, TC-006-TC-014'],['Negative testing','Confirm invalid or missing input is rejected','TC-002, TC-005'],['Security/navigation testing','Confirm authentication boundaries and logout','TC-015'],['Regression testing','Confirm existing application behavior remains stable','TC-018: 25 tests, 62 assertions'],['Visual validation','Confirm usability, status badges, forms and navigation','Figures A1-A5'],['Traceability review','Map tests to routes/controllers/database behavior','Section 6 and workbook register']],[1.6,2.5,2.5])

heading('4. Test Environment')
table(['Environment Item','Configuration'],[['Operating System','Microsoft Windows (local development workstation)'],['Application URL','http://127.0.0.1:8000'],['Browser','Chromium-based in-app browser, 1280 x 720 viewport'],['Application Server','Laravel development server on PHP'],['Framework','Laravel with Blade templates and Laravel Breeze authentication'],['Database','MySQL database: hotel_maintenance_db'],['Front End','Blade, Tailwind CSS/Vite assets and Font Awesome UI icons'],['Test Runner','PHPUnit through Laravel Artisan'],['Test Date','1 July 2026'],['Test Accounts','Administrator and maintenance staff accounts created for local validation'],['Test Data','Seeded requests in Pending, In Progress and Completed states']],[2.0,4.6])
...
labels=['Passed','Failed','Blocked']; values=[18,0,0]; colors=['#70AD47','#C00000','#A5A5A5']; chart=OUT/'test-results-chart.png'
...
table(['Status','Count','Percentage'],[['Pass','18','100%'],['Fail','0','0%'],['Blocked / Not Executed','0','0%'],['Total','18','100%']],[2.2,1.4,1.6])
p('The automated regression suite separately completed 25 tests with 62 assertions and zero failures. Functional cases and automated cases are reported separately to avoid double-counting coverage.')

heading('6. Requirements Traceability')
table(['Requirement Area','Implemented Component','Test Cases','Result'],[['Secure user access','Authentication routes and middleware','TC-001, TC-002, TC-015, TC-016','Pass'],['Operational overview','DashboardController and dashboard view','TC-003','Pass'],['Request lifecycle','MaintenanceController and maintenance_requests table','TC-004-TC-008, TC-017','Pass'],['Staff assignment','MaintenanceTask model/controller and assignment routes','TC-009','Pass'],['Task progression','Start and complete task routes','TC-010, TC-011','Pass'],['User notification','NotificationController and notifications table','TC-012, TC-013','Pass'],['Account maintenance','Profile and password controllers','TC-014, TC-016','Pass'],['Technical stability','Laravel route set and PHPUnit suite','TC-018','Pass']],[1.6,2.4,1.6,.7])
...
heading('8.4 Final Verification',2)
p('The final regression execution reported 25 passed tests and 62 assertions. Live navigation produced successful pages for the dashboard, maintenance list, request detail, request creation form and notification centre. No blocking server or browser error was observed in the final evidence pass.')

heading('9. Conclusion and Readiness')
p('The testing objectives were met. All 18 documented functional test cases passed, the automated suite passed 25 tests with 62 assertions, all in-scope requirements have traceable coverage, and no critical or major defect remains open. The system correctly supports the main hotel maintenance workflow from request submission through assignment, work progression, completion and notification.')
...
p('Recommendation: proceed with the CSC2854 final project demonstration using the seeded administrator and staff test accounts. Retain the accompanying workbook as the primary step-by-step test case record.')
...
p('The complete test case register is supplied as HMMS_FULL_TEST_CASES.xlsx. It contains a summary worksheet and 18 individual test case worksheets. Every worksheet records prerequisites, test data, three execution steps, expected results, actual results and final status.')
table(['Workbook Section','Content'],[['TEST SUMMARY','18 cases, module mapping, status and evidence reference'],['TC-001 to TC-002','Authentication positive and negative scenarios'],['TC-003','Dashboard verification'],['TC-004 to TC-008','Maintenance request lifecycle'],['TC-009 to TC-011','Assignment and task progression'],['TC-012 to TC-013','Notification behavior'],['TC-014','Profile update'],['TC-015 to TC-016','Protected navigation and password reset'],['TC-017','Request deletion'],['TC-018','Automated regression execution']],[2.1,4.3])
